...
function addPerson() {
  ...
  const addPersonModal = new bootstrap.Modal(
...
async function editPerson(element) {
  ...
  const header = element
    .closest(".person-container")
    .querySelector(".person-header");
  ...
  if (oldPersonName) {
    ...
    // Show the edit modal
    const editPersonModal = new bootstrap.Modal(
    ...
    saveButton.onclick = async function () {
      const editedPersonName = document
        .getElementById("editPersonName")
        .value.trim();
...
async function saveEditedPerson() {
  ...
  if (editedPersonName && currentPersonElement) {
    ...
    try {
      ...
      if (personIndex !== -1) {
        const response = await fetch(
          `/groups/${currentGroup}/persons/${oldPersonName}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
...
async function submitTask() {
  ...
  try {
    ...
    if (addToAll) {
      ...
      const response = await fetch(
        `/groups/${currentGroupId}/tasks/add-to-all`,
        {
          method: "POST",
          headers: {
      ...
      const response = await fetch(
        `/groups/${currentGroupId}/persons/${currentPersonId}/tasks`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
async function saveEditedGroup() {
  ...
  const currentGroups = Array.from(groupSelect.options).map(
    (option) => option.text
  );
  ...
  if (
    ...
    !currentGroups.includes(editedGroupName)
  ) {
    try {
...
function isDuplicatePerson(name) {
  return Array.from(container.getElementsByClassName("person-header")).some(
...
function isDuplicateTask(personName, taskName) {
  ...
  return Array.from(taskList.getElementsByTagName("task-name")).some(
    (task) => task.textContent.trim() === taskName
  );
}
  ...
  const cardHTML = `
        <div class="col-md-4">
            <div class="person-container">
                ...
                <div class="person-header" data-bs-toggle="collapse" data-bs-target="#collapse-${
                  person._id
                }" aria-expanded="false" aria-controls="collapse-${person._id}">
                    ${person.person_name}
                </div>
                ...
                <div class="btn-group">
                    <button data-group-id="${groupId}" data-id="${
    person._id
  }" class="btn-edit" onclick="editPerson(this)">
                        <i class="fas fa-pencil-alt"></i> Edit
                    </button>
                    <button data-group-id="${groupId}" data-id="${
    person._id
  }" class="btn-delete" onclick="deletePerson(this)">
                ...
                <button data-group-id="${groupId}" data-id="${
    person._id
  }" class="btn btn-task" onclick="addTask(this)">Add a Task</button>
                </div>            
        </div>
    `;
  ...
  return tasks
    .map((task) => {
      ...
      const formattedDoneDate = task.done_date
        ? new Date(task.done_date).toISOString().split("T")[0]
        : "";
      ...
      const taskNameStyle =
        task.status === "done"
          ...
          : isOverdue
          ? "color: red !important;"
          : ""; // Strike-through for task name if done
      const taskCalendarStyle = isOverdue
        ? "color: red !important; box-shadow: none;"
        : ""; // Strike-through for task name if done
      const todayDate = new Date().toISOString().split("T")[0];

      return `
            <div class="list-container" data-task-id="${
              task._id
            }" style="${taskCardStyle}">
                <div class="task-container">
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <div class="task-name" style="${taskNameStyle}">
                            <input type="checkbox" ${isChecked} onchange="toggleTaskStatus('${groupId}', '${personId}', '${
        task._id
      }', this)">
                            <div>
                                <div>${
                                ...
                                <div class="due-date text-muted" style="${taskCalendarStyle}">
                                    Due: <input type="date" style="${taskCalendarStyle}" 
                                            ...
                                            min="${todayDate}" 
                                            onchange="updateDueDate('${groupId}', '${personId}', '${
        task._id
      }', this)">
                                </div>
                                <div class="done-date text-muted" style="font-size: 12px">${
                                  formattedDoneDate
                                    ? `Done at: ${formattedDoneDate}`
                                    : ""
                                }</div>
                            </div>
                        </div>
                        <div class="d-flex align-items-center">
                            <button class="btn btn-primary favorite" onclick="toggleFavorite('${groupId}', '${personId}', '${
        task._id
      }', this)">
                                ${isFavorite}
                            </button>
                            <button class="btn-delete calendar" onclick="deleteTask('${groupId}', '${personId}', '${
        task._id
...
async function toggleTaskStatus(groupId, personId, taskId, checkbox) {
  ...
  try {
    const response = await fetch(
      `/groups/${groupId}/persons/${personId}/tasks/${taskId}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    if (result.success) {
      ...
      if (doneDateElement) {
        doneDateElement.textContent = doneDate
          ? "Done at: " + doneDate.toLocaleDateString()
          : "";
      }
...
function reorderTasks(groupId, personId) {
  ...
  const taskElements = Array.from(
    personContainer.querySelectorAll(".list-container")
  );
